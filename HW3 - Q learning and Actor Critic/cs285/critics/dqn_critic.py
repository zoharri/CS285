...
class DQNCritic(BaseCritic):
    ...
    def _build(self, q_func):
        ...
        if self.double_q:
            ...
            q_tp1_reuse = q_func(self.obs_tp1_ph, self.ac_dim, scope='q_func', reuse=True) #this is the updated q func
            arg_max_idx = tf.stack([tf.range(tf.shape(self.obs_t_ph)[0]), tf.cast(tf.argmax(q_tp1_reuse, axis=1),
                                                                                  tf.int32)], axis=1)
            q_tp1 = tf.gather_nd(q_tp1_values, arg_max_idx) #this is the target Q function from target_q_func scope
        else:
