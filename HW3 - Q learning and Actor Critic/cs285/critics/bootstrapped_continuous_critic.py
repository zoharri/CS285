...
class BootstrappedContinuousCritic(BaseCritic):
    ...
    def update(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for train_step in range(self.num_grad_steps_per_target_update*self.num_target_updates):
            if train_step % self.num_grad_steps_per_target_update == 0:
                vs_tp1 = self.forward(next_ob_no)*(1-terminal_n)
                target_v = re_n + self.gamma*vs_tp1
            _, loss = self.sess.run([self.critic_update_op, self.critic_loss], feed_dict={self.sy_ob_no: ob_no,self.sy_target_n: target_v})
        return loss
