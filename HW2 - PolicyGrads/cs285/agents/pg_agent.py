...
class PGAgent(BaseAgent):
    ...
    def train(self, obs, acs, rews_list, next_obs, terminals):
        ...
        loss = self.actor.update(obs, acs, qvals=q_values, adv_n=advantage_values) #Zohar
        return loss
    ...
    def estimate_advantage(self, obs, q_values):
        ...
        if self.nn_baseline:
            b_n_unnormalized = self.sess.run([self.actor.baseline_prediction], feed_dict={self.actor.observations_pl: obs})[0]
            b_n = b_n_unnormalized * np.std(q_values) + np.mean(q_values)
            adv_n = q_values.copy() - b_n
    ...
    def _discounted_return(self, rewards):
        ...
        assert np.shape(discounts) == np.shape(rewards) , "wrong shape in _discounted_return"
        ...
        list_of_discounted_returns = np.ones(len(discounted_rewards))*sum_of_discounted_rewards
        assert np.shape(list_of_discounted_returns) == np.shape(rewards), "wrong shape in _discounted_return"
